...
//     var cityName= await fetch('https://makeup-api.herokuapp.com/api/v1/products.json')
...
async function makeup() {
    ...
    try {
        ...
        for(i=1;i<=10;i++){
            ...
            pagination.setAttribute("id", 'btn'+[i]);
            pagination.innerHTML = i;
            document.getElementById('pg').appendChild(pagination);
            // pagination.setAttribute("onClick",`${paginationmethod("dheeraj")}`)
            document.getElementById('btn'+[i]).onclick = function() {paginationmethod(this.id)};
        }
        var next = document.createElement("button");
            next.setAttribute("id", 'next');
            document.getElementById('pg').appendChild(next);
            next.innerHTML = 'Next';
            let resultmethod = async (startcount,endcount) =>{
                console.log(">>>>",startcount,endcount)
                for (let i = startcount; i <= endcount; i++) {
                    // console.log("Count =>",startcount,endcount)
            
                    // var Name = await fetch(`https://makeup-api.herokuapp.com/api/v1/products.json?brand=maybelline`)
                    var result= await data
                    console.log(result)
                    var parentfirst = document.createElement("div");
                    parentfirst.setAttribute("id", 'name');
                    document.getElementById('container').appendChild(parentfirst);
                    var parent = document.createElement("div");
                     parent.setAttribute("id", 'id'+i);
                     parent.setAttribute("style", `display:flex`);
                    document.getElementById('name').appendChild(parent);
            
                     var name = document.createElement("div");
                     name.innerHTML = res[i].name
                     document.getElementById('id'+i).appendChild(name);
                     var brand = document.createElement("div");
                     brand.innerHTML = res[i].brand;
                     document.getElementById('id'+i).appendChild(brand);
                     var price = document.createElement("div");
                     price.innerHTML = res[i].price;
                     document.getElementById('id'+i).appendChild(price);
                     var image = document.createElement("div");
                     image.innerHTML = `<img src=${res[i]?.image_link} style="width:50px" />`;
                     document.getElementById('id'+i).appendChild(image);
                     name = res[i].name  ;
                     brand = res[i].brand;
                     price = res[i].price;
                     image = `<img src=${res[i]?.image_link} />`;
                }
                // document.getElementById("namess").innerHTML = name;
                // document.getElementById("brand").innerHTML = brand
                // document.getElementById("price").innerHTML = price
                // document.getElementById("image").innerHTML = image
            
                //     for (let key in element.image){
                // if(element.image[key].countries="flags"){
                //     console.log(element.)
                // }
            }
            let paginationmethod=(i)=>{
            ...
            let endcount = 5;
                ...
                // endcount=end;
                console.log("i =>",i)
                var ret = i.replace('btn','');
                startcount = (parseInt(ret) * 5) - 4;
                endcount = parseInt(ret) * 5;
                resultmethod(startcount,endcount)
            console.log(typeof parseInt(ret),startcount,endcount);  
            }
